#!/bin/bash
#SBATCH --job-name=MLP_ETT
#SBATCH --output=output_each_train_test_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=erro_each_train_testr_%j.txt    # Save error messages to a file named error_<job_id>.txt
