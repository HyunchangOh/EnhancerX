#!/bin/bash
#SBATCH --job-name=GB_L8
#SBATCH --output=outputGBL8_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorGBL8_%j.txt    # Save error messages to a file named error_<job_id>.txt
