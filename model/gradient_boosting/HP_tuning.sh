#!/bin/bash
#SBATCH --job-name=GB_HP
#SBATCH --output=result_HP_all_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=error_HP_all_%j.txt    # Save error messages to a file named error_<job_id>.txt
