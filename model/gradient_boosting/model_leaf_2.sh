#!/bin/bash
#SBATCH --job-name=GB_L2
#SBATCH --output=outputGBL2_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorGBL2_%j.txt    # Save error messages to a file named error_<job_id>.txt
