...
legacy_dir =  '/scratch/ohh98/la_grande_table_subsampled/subsampled_data/'
legacy_data_dir = os.path.join(legacy_dir,chr_folder)

response_file = os.path.join(legacy_data_dir, 'BIN50_enhancer_atlas.npy')
# model_save_dir = os.path.join(input_dir, 'saved_models', chr_folder)
model_save_dir = os.path.join(input_dir, 'sandbox', chr_folder)
...
for file in os.listdir(legacy_data_dir):
    if  file.startswith('BIN50_') and file.endswith('.npy') and (file != 'BIN50_enhancer_atlas.npy'  and not file.startswith("BIN50_enhancer_atlas")):
        ...
        features[feature_name] = np.load(os.path.join(legacy_data_dir, file))
