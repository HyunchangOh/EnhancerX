...
for i in range(niters):
    ...
    learning_rate_p = np.exp(np.random.normal(np.log(learning_rate),sigma_l))                                 # <-----------

    while learning_rate_p<0.0005:
        learning_rate_p = np.exp(np.random.normal(np.log(learning_rate),sigma_l))
    ...
    # Q(theta|theta')
    Q_Current_from_proposed = proposal_prob(learning_rate,learning_rate_p,sigma_l)*proposal_prob(min_leaves,min_leaves_p,sigma_p)*proposal_prob(max_depth,max_depth_p,sigma_p)
    #Q(theta'|theta)
    Q_Proposed_from_current = proposal_prob(learning_rate_p,learning_rate,sigma_l)*proposal_prob(min_leaves_p,min_leaves,sigma_p)*proposal_prob(max_depth_p,max_depth,sigma_p)
    ...
    posterior_ratio = ((roc_auc_average_p-0.5)/(roc_auc_average-0.5))**2
    ...
    if u < rho:
        ...
        accepted_recalls.append(recalls)
        accepted_roc_auc_scores.append(roc_auc_scores)
        accepted_parameters.append([learning_rate,min_leaves,max_depth])
        if roc_auc_best <roc_auc_average_p:
            roc_auc_best = roc_auc_average_p
        roc_auc_average = roc_auc_average_p

        print(model_index,roc_auc_average)
        print(learning_rate,min_leaves,max_depth)
