#!/bin/bash
#SBATCH --job-name=GB_10
#SBATCH --output=outputGB10_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorGB10_%j.txt    # Save error messages to a file named error_<job_id>.txt
