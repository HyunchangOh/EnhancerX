#!/bin/bash
#SBATCH --job-name=LR_all
#SBATCH --output=outputLR_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorLR_%j.txt    # Save error messages to a file named error_<job_id>.txt
