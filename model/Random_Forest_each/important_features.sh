#!/bin/bash
#SBATCH --job-name=GB_IF
#SBATCH --output=outputIF_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorIF_%j.txt    # Save error messages to a file named error_<job_id>.txt
...
#SBATCH --mem=4G                # Specify the total memory required for the job (example: 4GB)
