#!/bin/bash
#SBATCH --job-name=RF_200
#SBATCH --output=output200_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=error200_%j.txt    # Save error messages to a file named error_<job_id>.txt
