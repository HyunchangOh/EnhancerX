#!/bin/bash
#SBATCH --job-name=GB_LG
#SBATCH --output=sandbox_z_results%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=sandbox_z_error%j.txt    # Save error messages to a file named error_<job_id>.txt
