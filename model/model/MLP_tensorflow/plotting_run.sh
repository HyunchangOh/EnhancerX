#!/bin/bash
#SBATCH --job-name=run_plotting
#SBATCH --output=output_pretty_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=error_pretty_%j.txt    # Save error messages to a file named error_<job_id>.txt
