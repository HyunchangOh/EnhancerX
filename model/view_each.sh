#!/bin/bash
#SBATCH --job-name=
#SBATCH --output=outputRF_%j.txt  # Save output to a file named output_<job_id>.txt
#SBATCH --error=errorRf_%j.txt    # Save error messages to a file named error_<job_id>.txt
