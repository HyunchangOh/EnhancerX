...
save_path = "../../../../../scratch/ohh98/vingron3D/"
for c in chromosomes:
    chr_path = data_path+c+"/"
    enhancer_boolean_array = np.load(chr_path+"promoter_any.npy")
    enhancer_distances_array = np.load(chr_path+"promoter_3D_Dist_any.npy")
    for f in features:
        ...
        plt.savefig(save_path+c+"/Promoter"+f+str(frame_size)+".png")
        plt.close()
        np.save(save_path+c+"/Promoter"+f+str(frame_size)+".npy",distances)
...
for f in features:
    ...
    for c in chromosomes:
        dist = np.load(save_path+c+"/Promoter"+f+str(frame_size)+".npy")
        overall_distribution = np.concatenate((overall_distribution,dist))
    np.save(save_path+"overall"+"/Promoter"+f+str(frame_size)+".npy", overall_distribution)
    ...
    plt.savefig(save_path+"overall"+"/Promoter"+f+str(frame_size)+".png")
    plt.close()
